...
var NhanVien = function () {
    ...
    let initialComponents = () => {
        $('#filterByGenderId').change(function (e) {
            dtTable.draw();
        });
        $('#filterByBuuCucId').change(function (e) {
            dtTable.draw();
        });
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            window.location.href = "/NhanVien/Edit";
        });
        ...
        App.initSelect2Base($('#filterByBuuCucId'), '/BuuCuc/Filter', {
            query: {
                ...
                filterByHuyenId: function () {
                    return $('#filterByHuyenId').val() ? $('#filterByHuyenId').val() : null;
                }
            }
        });
    };
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/NhanVien/DataTableGetList";
        datatableOption.ajax.data = {
            textSearch: function () {
                return $('#dtTableSearch').val();
            },
            filterByBuuCucId: function () {
                return $('#filterByBuuCucId').val() ? $('#filterByBuuCucId').val() : null;
            },
            filterByGenderId: function () {
                return $('#filterByGenderId').val() ? $('#filterByGenderId').val() : null;
            }
        };
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã nhân viên" },
            { "data": "fullName", "name": "fullName", "autoWidth": true, "title": "Tên nhân viên" },
            { "data": "diaChi", "name": "diaChi", "autoWidth": true, "title": "Địa chỉ" },
            { "data": "soDienThoai", "name": "soDienThoai", "autoWidth": true, "title": "Số điện thoại" },
            {
                "data": "gender", "name": "gender", "autoWidth": true, "title": "Giới tính", "render": function (data, type, full, meta) {
            ...
            },
            {
                width: "120px", "title": "Hành động", "render": function (data, type, full, meta) {
                    ...
                    if (user.roles.isAllowEdit == true) {
                        html += '<a asp-controller="NhanVien" asp-action="Edit" type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 edit">\
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            if (selectedDataRow != null) {
                window.location.href = "/NhanVien/Edit?id=" + selectedDataRow.id;
            }
        });
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/NhanVien/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
