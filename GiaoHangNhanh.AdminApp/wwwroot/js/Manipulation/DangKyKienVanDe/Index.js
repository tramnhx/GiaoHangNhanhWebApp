...
var DangKyKienVanDe = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_vanDonId: { validators: { notEmpty: { message: "vui lòng chọn mã vận đơn" } } },
            kt_modal_edit_form_phanLoaiHangBatThuongId: { validators: { notEmpty: { message: "vui lòng chọn phân loại hàng bất thường" } } },
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm Kiện Vấn Đề");
        ...
        $('#kt_modal_edit_submit').click(function (e) {
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/DangKyKienVanDe/Save", data, "POST")
                                .then(function (res) {
            ...
            $('input[name="kt_modal_edit_form_vanDonId"]').focus();
        });
        ...
        // modal add
        App.initSelect2Base($('[name="kt_modal_edit_form_phanLoaiHangBatThuongId"]'), '/PhanLoaiHangBatThuong/Filter', { selectedFields: ["id", "name"] });
        App.initSelectCodeBase($('[name="kt_modal_edit_form_vanDonId"]'), '/VanDon/Filter');

        $('[name="kt_modal_edit_form_vanDonId"]').change(function () {
            loadDiaChiByVanDon();
            loadHoTenNguoiGuiByVanDon();
            loadHoTenNguoiNhanByVanDon();
            loadDienThoaiNguoiGuiByVanDon();
            loadDienThoaiNguoiNhanByVanDon();
            loadNgayGuiHangByVanDon();
            loadBuuCucByVanDon();
            loadCongTyGuiHangByVanDon();
        });
    ...
    function loadHoTenNguoiNhanByVanDon() {
        $('[name="kt_modal_edit_form_hoTenNguoiNhan"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vanDonId"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.hoTenNguoiNhan + '</option>';
                    $('[name="kt_modal_edit_form_hoTenNguoiNhan"]').append(html);
                    $('[name="kt_modal_edit_form_hoTenNguoiNhan"]').trigger('change');
                }
            });
        }
    }

    function loadDienThoaiNguoiGuiByVanDon() {
        $('[name="kt_modal_edit_form_dienThoaiNguoiGui"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vanDonId"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.dienThoaiNguoiGui + '</option>';
                    $('[name="kt_modal_edit_form_dienThoaiNguoiGui"]').append(html);
                    $('[name="kt_modal_edit_form_dienThoaiNguoiGui"]').trigger('change');
                }
            });
        }
    }

    function loadDienThoaiNguoiNhanByVanDon() {
        $('[name="kt_modal_edit_form_dienThoaiNguoiNhan"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vanDonId"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.dienThoaiNguoiNhan + '</option>';
                    $('[name="kt_modal_edit_form_dienThoaiNguoiNhan"]').append(html);
                    $('[name="kt_modal_edit_form_dienThoaiNguoiNhan"]').trigger('change');
                }
            });
        }
    }
    ...
    function loadBuuCucByVanDon() {
        $('[name="kt_modal_edit_form_buuCuc"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vanDonId"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.buuCuc.name + '</option>';
                    $('[name="kt_modal_edit_form_buuCuc"]').append(html);
                    $('[name="kt_modal_edit_form_buuCuc"]').trigger('change');
                }
            });
        }
    }

    function loadCongTyGuiHangByVanDon() {
        $('[name="kt_modal_edit_form_congTyGuiHang"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vanDonId"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.congTyGuiHang.name + '</option>';
                    $('[name="kt_modal_edit_form_congTyGuiHang"]').append(html);
                    $('[name="kt_modal_edit_form_congTyGuiHang"]').trigger('change');
                }
            });
        }
    }
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/DangKyKienVanDe/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            {
                "data": "vanDon", "name": "vanDon", "autoWidth": true, "title": "Điện thoại người gửi", "render": function (data, type, full, meta) {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('[name="kt_modal_edit_form_vanDonId"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.code + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_phanLoaiHangBatThuongId"]').append("<option value=" + selectedDataRow.phanLoaiHangBatThuong.id + " selected>" + selectedDataRow.phanLoaiHangBatThuong.name + "</option>").trigger('change');
            ...
            $('[name="kt_modal_edit_form_hoTenNguoiNhan"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.hoTenNguoiNhan + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_dienThoaiNguoiGui"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.dienThoaiNguoiGui + "</option>").trigger('change');
            ...
            $('[name="kt_modal_edit_form_buuCuc"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.buuCuc.name + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_congTyGuiHang"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.congTyGuiHang.name + "</option>").trigger('change');

            modal_header_text.text("Cập nhật đăng ký kiện vấn đề");
            modal_form_submit_text.text("Cập nhật");
            setTimeout(function () { $('input[name="kt_modal_edit_form_vanDonId"]').focus() }, 500);
            modal.modal('show');
    ...
    let resetForm = () => {
        modal_form[0].reset();
        $('[name="kt_modal_edit_form_phanLoaiHangBatThuongId"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_vanDonId"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_hoTenNguoiGui"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_hoTenNguoiNhan"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_dienThoaiNguoiGui"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_ngayGuiHang"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_buuCuc"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_congTyGuiHang"]').val(null).trigger('change');
        setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500);
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/DangKyKienVanDe/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
