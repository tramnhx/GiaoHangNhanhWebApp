...
var LichSuDongBao = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_vandon_id: { validators: { notEmpty: { message: "vui lòng chọn mã vận đơn" } } },
            kt_modal_edit_form_seal: { validators: { notEmpty: { message: "vui lòng nhập seal bao" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm Bao");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/LichSuDongBao/Save", data, "POST")
                                .then(function (res) {
    ...
    function loadDichVuByVanDon() {
        $('[name="kt_modal_edit_form_dichvu"]').empty();
        if ($('[name="kt_modal_edit_form_vandon_id"]').val() != null) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.dichVu.name + '</option>';
                    $('[name="kt_modal_edit_form_dichvu"]').append(html);
                    $('[name="kt_modal_edit_form_dichvu"]').trigger('change');
                }
            });
        }
    }

    function loadMoTaByVanDon() {
        $('[name="kt_modal_edit_form_mota_hanghoa"]').empty();
        if ($('[name="kt_modal_edit_form_vandon_id"]').val() != null) {
            var data = {
                id: $('[name="kt_modal_edit_form_vandon_id"]').val()
            };
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.noiDungHangHoa + '</option>';
                    $('[name="kt_modal_edit_form_mota_hanghoa"]').append(html);
                    $('[name="kt_modal_edit_form_mota_hanghoa"]').trigger('change');
                }
            });
        }
    }
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/LichSuDongBao/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            {
                "data": "vanDon", "name": "vanDon", "autoWidth": true, "title": "Dịch vụ", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data.dichVu.name + '</span>';
                }
            },
            { "data": "sealBao", "name": "sealBao", "autoWidth": true, "title": "Seal bao" },
            {
                "data": "vanDon", "name": "vanDon", "autoWidth": true, "title": "Mô tả hàng hóa", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data.noiDungHangHoa + '</span>';
                }
            },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('[name="kt_modal_edit_form_mota_hanghoa"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.noiDungHangHoa + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_dichvu"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.dichVu.name + "</option>").trigger('change');

            modal_header_text.text("Cập nhật bao");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/LichSuDongBao/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
