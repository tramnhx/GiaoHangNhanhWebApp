...
var LichSuGuiHang = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            ...
            kt_modal_edit_form_buuCucId: { validators: { notEmpty: { message: "vui lòng chọn trạm sau" } } },
            kt_modal_edit_form_khuVucHangDenId: { validators: { notEmpty: { message: "vui lòng chọn khu vực hàng đến" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm gửi hàng");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/LichSuGuiHang/Save", data, "POST")
                                .then(function (res) {
        ...
        $('[name="kt_modal_edit_form_vanDonId"]').change(function () {
            loadBuuCucHangDenByVanDon();
        });
    };

    function loadBuuCucHangDenByVanDon() {
        $('[name="kt_modal_edit_form_khuVucHangDenId"]').empty();
        if ($('[name="kt_modal_edit_form_vanDonId"]').val() != null) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    var html = '<option value="' + res.resultObj.id + '">' + res.resultObj.buuCuc.code + '</option>';
                    $('[name="kt_modal_edit_form_khuVucHangDenId"]').append(html);
                    $('[name="kt_modal_edit_form_khuVucHangDenId"]').trigger('change');
                }
            });
        }
    }
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/LichSuGuiHang/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            {
                "data": "buuCuc", "name": "buuCuc", "autoWidth": true, "title": "Trạm sau", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data.name + '</span>';
                }
            },
            {
                "data": "vanDon", "name": "vanDon", "autoWidth": true, "title": "Mã khu vực hàng đến", "render": function (data, type, full, meta) {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('[name="kt_modal_edit_form_khuVucHangDenId"]').append("<option value=" + selectedDataRow.vanDon.id + " selected>" + selectedDataRow.vanDon.buuCuc.code + "</option>").trigger('change');

            modal_header_text.text("Cập nhật gửi hàng");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/LichSuGuiHang/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
