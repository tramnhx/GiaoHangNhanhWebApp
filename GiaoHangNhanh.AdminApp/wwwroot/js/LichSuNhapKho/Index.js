...
var LichSuNhapKho = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            ...
            kt_modal_edit_form_buucucId: { validators: { notEmpty: { message: "vui lòng chọn Bưu Cục Gửi Hàng" } } },
            kt_modal_edit_form_UserId: { validators: { notEmpty: { message: "vui lòng chọn nhân viên lấy hàng" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('#kt_filter_form_button_reset').click(function (e) {
            ...
            $('#filterByUserId').val(null).trigger('change');
        });
        ...
        App.initSelect2Base($('#filterByUserId'), '/User/Filter', {
            query: {
                filterByBuuCucId: function () {
                    return $('#filterByBuuCucId').val() ? $('#filterByBuuCucId').val() : '';
                }
            }
        });
        App.initSelect2Base($('#filterByLichSuNhapKhoId'), '/LichSuNhapKho/Filter', {
            query: {
                filterByBuuCucId: function () {
                    return $('#filterByBuuCucId').val() ? $('#filterByBuuCucId').val() : '';
                },
                filterByUserId: function () {
                    return $('#filterByUserId').val() ? $('#filterByUserId').val() : '';
                }
            }
        });
        // modal add
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/LichSuNhapKho/Save", data, "POST")
                                .then(function (res) {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/LichSuNhapKho/DataTableGetList";
        datatableOption.ajax.data = {
            textSearch: function () {
                return $('#dtTableSearch').val();
            },
            filterByBuuCucGuiHangId: function () {
                return $('#filterByBuuCucId').val();
            },
            filterByUserId: function () {
                return $('#filterByUserId').val();
            }
        };
        ...
        datatableOption.columns = [
            ...
            {
                "data": "buuCuc", "name": "buuCuc", "autoWidth": true, "title": "Bưu cục gửi hàng", "render": function (data, type, full, meta) {
...
<path d="M14,4.5 L14,4 C14,3.44771525 13.5522847,3 13,3 L11,3 C10.4477153,3 10,3.44771525 10,4 L10,4.5 L5.5,4.5 C5.22385763,4.5 5,4.72385763 5,5 L5,5.5 C5,5.77614237 5.22385763,6 5.5,6 L18.5,6 C18.7761424,6 19,5.77614237 19,5.5 L19,5 C19,4.72385763 18.7761424,4.5 18.5,4.5 L14,4.5 Z" fill="#000000" opacity="0.3"></path>\
    ...
    let resetForm = () => {
        ...
        $('[name="kt_modal_edit_form_userId"]').val(null).trigger('change');
        setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500); 
        $('[name="kt_modal_edit_form_buucucId"]').val(null).trigger('change');
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/LichSuNhapKho/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
