...
var Menu = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            ...
            kt_modal_edit_form_controllerName: { validators: { notEmpty: { message: "vui lòng nhập tên controller" } } },
            kt_modal_edit_form_actionName: { validators: { notEmpty: { message: "vui lòng nhập tên action" } } },
            kt_modal_edit_form_sortOrder: { validators: { notEmpty: { message: "vui lòng nhập sắp xếp" } } },
            plugins: {
    ...
    let initialComponents = () => {
        ...
        $('button[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một");
            modal_form_submit_text.text("Thêm");
            resetForm();
            modal.modal("show");
        });
        // filter
        App.initSelect2Base($('#filterByTinhId'), '/Menu/Filter');
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/Menu/Save", data, "POST")
                                .then(function (res) {
        ...
        App.initSelect2Base($('[name="kt_modal_edit_form_parentId"]'), '/Menu/Filter');
    };
    ...
    let initialDatatable = function () {
        ...
        datatableOption.columns = [
            ...
            { "data": "sortOrder", "name": "sortOrder", "autoWidth": true, "title": "sortOrder" },
            ...
            {
                "data": "controllerName", "name": "controllerName", "autoWidth": true, "title": "Tên controller", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "actionName", "name": "actionName", "autoWidth": true, "title": "Tên action", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "icon", "name": "icon", "autoWidth": true, "title": "Icon", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "description", "name": "description", "autoWidth": true, "title": "Mô tả", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="ControllerName"]').val(selectedDataRow.controllerName);
            $('input[data-field="ActionName"]').val(selectedDataRow.actionName);
