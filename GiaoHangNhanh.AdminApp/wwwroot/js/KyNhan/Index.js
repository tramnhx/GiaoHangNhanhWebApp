...
var KyNhan = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã ký nhận" } } },
            kt_modal_edit_form_vanDonId: { validators: { notEmpty: { message: "vui lòng nhập mã vận đơn" } } },
            kt_modal_edit_form_maBuCuc: { validators: { notEmpty: { message: "vui lòng nhập mã bưu cục" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên ký nhận" } } },
        },
        plugins: {
    ...
    let initialComponents = () => {
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/KyNhan/Save", data, "POST")
                                .then(function (res) {
        ...
        // filter
        App.initSelect2Base($('#filterByBuuCuc'), '/BuuCuc/Filter');
        initSelectUserBase($('[name="kt_modal_edit_form_nhanVienId"]'), '/Staff/Filter');

        //App.initSelect2Base($('#filterByMaVanDon'), '/VanDon/Filter');
    ...
    let initSelectUserBase = (el, url, options) => {
                    ...
                    processResults: function (res) {
                        ...
                        var data = $.map(res.items, function (item, i) {
                            ...
                            return {
                                ...
                                text: (options && options.selectedFields ? item[options.selectedFields[1]] : item.fullName)
                            }
                        });
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/KyNhan/DataTableGetList";
        datatableOption.ajax.data = {
            textSearch: function () {
                return $('#dtTableSearch').val();
            },
            /*filterByDMBuuCuc: function () {
                return $('#FilterByDMBuuCuc').val() ? $('#filterByBuuCuc').val() : 0;
            }*/
        };
        ...
        datatableOption.columns = [
            ...
            {
                "data": "buuCuc", "name": "buuCuc", "autoWidth": true, "title": "Mã bưu cục", "render": function (data, type, full, meta) {
                    return '<span class="font-monospace">' + data.code + '</span>';
                },
            },
            ...
            { "data": "ngayKyNhan", "name": "ngayKyNhan", "autoWidth": true, "title": "Thời gian ký" },
            {
                "data": "vanDon", "name": "vanDon", "autoWidth": true, "title": "Nhân viên phát", "render": function (data, type, full, meta) {
                    return '<span class="font-monospace">' + data.nhanVien.fullName + '</span>';
                }
            },
            { "data": "description", "name": "description", "autoWidth": true, "title": "Ghi chú" },
            {
                "data": "dauKyThay", "name": "dauKyThay", "autoWidth": true, "title": "Ký thay",
            ...
            { "data": "tenNguoiKy", "name": "tenNguoiKy", "autoWidth": true, "title": "Người ký nhận" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('[name="kt_modal_edit_form_buuCucId"]').append("<option value=" + selectedDataRow.buuCuc.id + " selected>" + selectedDataRow.buuCuc.code + "</option>").trigger('change');
            ...
            $('[name="kt_modal_edit_form_nhanVienId"]').append("<option value=" + selectedDataRow.vanDon.user.id + " selected>" + selectedDataRow.vanDon.user.fullName + "</option>").trigger('change');

            $('input[data-field="DauKyThay"]').prop('checked', editingDataRow.dauKyThay);
            $('input[data-field="Description"]').val(selectedDataRow.description);
            $('input[data-field="TenNguoiKy"]').val(selectedDataRow.tenNguoiKy);
    ...
    let resetForm = () => {
        ...
        $('[name="kt_modal_edit_form_nhanVienId"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_tennguoiky"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_description"]').val(null).trigger('change');

    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/KyNhan/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
