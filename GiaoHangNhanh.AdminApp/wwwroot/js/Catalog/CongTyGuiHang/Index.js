...
var CongTyGuiHang = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã công ty gửi hàng" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên công ty gửi hàng" } } },
            kt_modal_edit_form_phoneNumber: { validators: { notEmpty: { message: "vui lòng nhập số điện thoại công ty gửi hàng" } } },
            kt_modal_edit_form_tinhId: { validators: { notEmpty: { message: "vui lòng chọn tỉnh" } } },
            kt_modal_edit_form_huyenId: { validators: { notEmpty: { message: "vui lòng chọn huyện" } } },
            kt_modal_edit_form_khuVucId: { validators: { notEmpty: { message: "vui lòng chọn khu vực" } } },
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một công ty gửi hàng");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/CongTyGuiHang/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/CongTyGuiHang/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã Công ty" },
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên Công ty" },
            { "data": "phoneNumber", "name": "phoneNumber", "autoWidth": true, "title": "Số điện thoại" },
            { "data": "diaChi", "name": "diaChi", "autoWidth": true, "title": "Địa chỉ" },
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('[name="kt_modal_edit_form_tinhId"]').append("<option value=" + selectedDataRow.tinh.id + " selected>" + selectedDataRow.tinh.name + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_huyenId"]').append("<option value=" + selectedDataRow.huyen.id + " selected>" + selectedDataRow.huyen.name + "</option>").trigger('change');
            $('[name="kt_modal_edit_form_khuVucId"]').append("<option value=" + selectedDataRow.khuVuc.id + " selected>" + selectedDataRow.khuVuc.name + "</option>").trigger('change');
            ...
            modal_header_text.text("Cập nhật Công ty gửi hàng");
    ...
    let resetForm = () => {
        modal_form[0].reset();
        $('[name="kt_modal_edit_form_tinhId"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_huyenId"]').val(null).trigger('change');
        $('[name="kt_modal_edit_form_khuVucId"]').val(null).trigger('change');
        setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500);
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/CongTyGuiHang/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
