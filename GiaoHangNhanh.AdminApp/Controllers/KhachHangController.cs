...
namespace GiaoHangNhanh.AdminApp.Controllers
{
    ...
    {
        private readonly IKhachHangApiClient _khachHangApiClient;
        public KhachHangController(IKhachHangApiClient khachHangApiClient)
        {
            _khachHangApiClient = khachHangApiClient;
        }
        public IActionResult Index()
        {
            ...
            model.Breadcrumbs = new List<string>() { "Cài đặt", "Danh mục hệ thống", "Khách hàng" };
        ...
        public async Task<IActionResult> DataTableGetList(int? draw, int? start, int? length, string textSearch)
        {
            ...
            var request = new ManageKhachHangPagingRequest()
            {
            ...
            var khachHangApiClient = await _khachHangApiClient.GetManageListPaging(request);

            return Json(new
            {
                draw = draw,
                recordsFiltered = khachHangApiClient.TotalRecords,
                recordsTotal = khachHangApiClient.TotalRecords,
                data = khachHangApiClient.Items
            });
        }
        ...
        {
            return Json(await _khachHangApiClient.DeleteByIds(request));
        }

        [HttpPost]
        public async Task<IActionResult> Save([FromBody] AddEditRequest<KhachHangRequest> rq)
        {
            ...
            if (rq != null)
            {
                ...
                result = await _khachHangApiClient.AddOrUpdateAsync(rq.Data);
            }
            else
            {
        ...
        public async Task<IActionResult> Filter(string textSearch)
        {
            ...
            var data = await _khachHangApiClient.GetManageListPaging(request);
            return Ok(data);
        }
    }
}
